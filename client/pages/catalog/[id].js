                                ...
                                <div className="spls_style_two mb30-520" style={{display: 'flex', justifyContent: 'center'}}>
                                    {
                                        data?.images?.length > 0 ? (
                                            <a className="popup-img" href={`${process.env.API_HOST}/${data.images?.find(item => {return item.main == true}) ? data.images?.find(item => {return item.main == true})?.src : data?.images?.[0]?.src}`}><img className="img-fluid" style={{maxHeight: 600, objectFit: 'scale-down', borderRadius: '10px'}} src={`${process.env.API_HOST}/${data.images?.find(item => {return item.main == true}) ? data.images?.find(item => {return item.main == true})?.src : data?.images?.[0]?.src}`} alt="1.jpg"></img></a>
                                        ) : (
                                ...
                                data.images?.filter(item => {return !item.main})?.map(image => (
                                    <div className="col-sm-6 col-lg-6">
                                        <div className="spls_style_two mb30">
                                            <a className="popup-img" href={`${process.env.API_HOST}/${image.src}`}><img className="img-fluid" style={{maxHeight: 200, objectFit: 'scale-down'}} src={`${process.env.API_HOST}/${image.src}`} alt="2.jpg"></img></a>
                                        </div>
                                    </div>
                                ))
                            }
